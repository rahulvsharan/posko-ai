...
describe("models registry", () => {
  test("static catalog has 26 models (7 opencode + 19 kilo)", () => {
    setAliveIds(null);
    expect(ALL_MODELS.length).toBe(26);
    expect(ALL_MODELS.filter((m) => m.upstream === "opencode").length).toBe(7);
    expect(ALL_MODELS.filter((m) => m.upstream === "kilo").length).toBe(19);
  });
